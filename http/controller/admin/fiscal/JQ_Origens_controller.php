...
if ($_POST["JQueryFunction"] == 'novaOrigem') {
    $response = array();

    $codigo = $_POST['codigo'];
    $descricao = $_POST['descricao'];

    $query = "INSERT INTO FIS_ORIGENS_PRODUTO (CODIGO, DESCRICAO, INCLUIDOPOR, INCLUIDOEM)
...
if ($_POST["JQueryFunction"] == 'editarOrigem') {
    $response = array();
    ...
    $query = "UPDATE FIS_ORIGENS_PRODUTO SET
        DESCRICAO = '$descricao',
        CODIGO = '$codigo',
    ...
    if (mysqli_query($conexao, $query)) {
        ...
        $selectQuery = "SELECT DESCRICAO, CODIGO, ATIVO FROM FIS_ORIGENS_PRODUTO 
        WHERE ID = '$id'";
...
if ($_POST["JQueryFunction"] == 'inativarOrigem') {
    $response = array();
    ...
    if (mysqli_query($conexao, $query)) {
        // Após o update, buscar os dados atualizados incluindo a coluna ATIVO
        $selectQuery = "SELECT CODIGO, DESCRICAO, ATIVO FROM FIS_ORIGENS_PRODUTO WHERE ID = '$id'";
...
if ($_POST["JQueryFunction"] == 'deletarOrigem') {
    $response = array();

    $id = $_POST['id'];

    $query = "DELETE FROM FIS_ORIGENS_PRODUTO WHERE ID = '$id'";
