...
if ($_POST["JQueryFunction"] == 'cadastrarFormasDePagamento') {
    $response = array();

    $descricao = $_POST['descricao'];

    $query = "INSERT INTO FIN_FORMAS_DE_PAGAMENTO (DESCRICAO, INCLUIDOPOR, INCLUIDOEM, EMPRESA_ID)
...
if ($_POST["JQueryFunction"] == 'editarFormaDePagamento') {
    $response = array();

    $descricao = $_POST['descricao'];
    $id = $_POST['id'];

    $query = "UPDATE FIN_FORMAS_DE_PAGAMENTO 
    SET 
        DESCRICAO = '$descricao',
    ...
    if (mysqli_query($conexao, $query)) {
        ...
        $selectQuery = "SELECT DESCRICAO, ATIVO FROM FIN_FORMAS_DE_PAGAMENTO 
        WHERE ID = '$id' AND EMPRESA_ID = '$IDEMPRESAUSUARIOMODEL'";
...
if ($_POST["JQueryFunction"] == 'inativarFormaPagamento') {
    $response = array();
    ...
    $query = "UPDATE FIN_FORMAS_DE_PAGAMENTO SET
        ATIVO = '$status',
        ALTERADOPOR = '$IDUSUARIOMODEL',
        ALTERADOEM = NOW()
    WHERE ID = '$id' AND EMPRESA_ID = '$IDEMPRESAUSUARIOMODEL'";
...
if ($_POST["JQueryFunction"] == 'deletarFormaPagamento') {
    $response = array();

    $id = $_POST['id'];

    $query = "DELETE FROM FIN_FORMAS_DE_PAGAMENTO WHERE ID = '$id' AND EMPRESA_ID = '$IDEMPRESAUSUARIOMODEL'";
